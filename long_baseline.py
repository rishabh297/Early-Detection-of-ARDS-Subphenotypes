...
# Filter the DataFrame to include only events within 24-48 hours of admission
merged_df_day2 = merged_df[(merged_df['time_diff_hours'] > 24) & (merged_df['time_diff_hours'] <= 48)]

# Day 3

# Filter the DataFrame to include only events within 48-72 hours of admission
merged_df_day3 = merged_df[(merged_df['time_diff_hours'] > 48) & (merged_df['time_diff_hours'] <= 72)]
...
# Filter data
merged_df_day2= merged_df_day2[merged_df_day2['itemid'].isin(item_ids)]


# Find rows with minimum charttime
idx = merged_df_day2.groupby(['subject_id', 'hadm_id', 'itemid'])['charttime'].idxmin()
merged_df_day2 = merged_df_day2.loc[idx]

# Pivot DataFrame
merged_df_day2 = merged_df_day2.pivot_table(index=['subject_id', 'hadm_id'], columns='itemid', values='value')
merged_df_day2.reset_index(inplace=True)

print(merged_df_day2)

# Filter data
merged_df_day3= merged_df_day3[merged_df_day3['itemid'].isin(item_ids)]
merged_df_day3['charttime'] = pd.to_datetime(merged_df_day3['charttime'])

# Find rows with minimum charttime
idx = merged_df_day3.groupby(['subject_id', 'hadm_id', 'itemid'])['charttime'].idxmin()
merged_df_day3 = merged_df_day3.loc[idx]

# Pivot DataFrame
merged_df_day3 = merged_df_day3.pivot_table(index=['subject_id', 'hadm_id'], columns='itemid', values='value')
merged_df_day3.reset_index(inplace=True)
...
merged_df_day2.rename(columns=item_id_dict, inplace=True)

merged_df_day3.rename(columns=item_id_dict, inplace=True)
...
missing_data_percentage = (merged_df_day2.isnull().sum() / len(pivot_df)) * 100
print("Missing data percentages before removal day 2:")
print(missing_data_percentage.head(60))

missing_data_percentage = (merged_df_day3.isnull().sum() / len(pivot_df)) * 100
print("Missing data percentages before removal day 3:")
print(missing_data_percentage.head(60))


# Calculate missing data percentage before removal
missing_data_percentage = (pivot_df.isnull().sum() / len(pivot_df)) * 100
print("Missing data percentages before removal day 1:")
print(missing_data_percentage.head(60))
...
pivot_df_mice = pivot_df_mice.drop(columns=['anchor_age'])

# get the list of column names from the day 1 dataframe
day1_columns = pivot_df_mice.columns.tolist()

# Fit and transform the dataframe to fill NaN values
merged_df_day2_mice = mice_imputer.fit_transform(merged_df_day2)
merged_df_day2 = pd.DataFrame(merged_df_day2_mice, columns=merged_df_day2.columns)

merged_df_day2 = merged_df_day2[day1_columns]



# Merge hospital_stay_duration_df with expired_admissions
merged_df_day2_unscaled = merged_df_day2.merge(pivot_df[['subject_id', 'hadm_id', 'hospital_expire_flag', 'mortality_30d', 'mortality_90d']], on=['subject_id', 'hadm_id'], how='inner')

merged_df_day2_unscaled = merged_df_day2_unscaled.merge(copy_for_time_until_death[['subject_id', 'hadm_id', 'time_until_death']], on=['subject_id', 'hadm_id'], how='inner')

merged_df_day2_unscaled.to_csv('/Users/rishabhgoel/Desktop/Gen1E-RIDGE/MIMIC-IV Project and eICU-CRD/longitudinal/mice_filled_baseline_preprocessed_out_day2.csv', index=False)


merged_df_day3 = merged_df_day3[day1_columns]

# Fit and transform the dataframe to fill NaN values
merged_df_day3_mice = mice_imputer.fit_transform(merged_df_day3)
merged_df_day3 = pd.DataFrame(merged_df_day3_mice, columns=merged_df_day3.columns)

# Merge hospital_stay_duration_df with expired_admissions
merged_df_day3_unscaled = merged_df_day3.merge(pivot_df[['subject_id', 'hadm_id', 'hospital_expire_flag', 'mortality_30d', 'mortality_90d']], on=['subject_id', 'hadm_id'], how='inner')

merged_df_day3_unscaled = merged_df_day3_unscaled.merge(copy_for_time_until_death[['subject_id', 'hadm_id', 'time_until_death']], on=['subject_id', 'hadm_id'], how='inner')

merged_df_day3_unscaled.to_csv('/Users/rishabhgoel/Desktop/Gen1E-RIDGE/MIMIC-IV Project and eICU-CRD/longitudinal/mice_filled_baseline_preprocessed_out_day3.csv', index=False)

# Merge hospital_stay_duration_df with expired_admissions
pivot_df_mice = pivot_df_mice.merge(merged_df_day2[['subject_id', 'hadm_id']], on=['subject_id', 'hadm_id'], how='inner')
...
pivot_df_unscaled.to_csv('/Users/rishabhgoel/Desktop/Gen1E-RIDGE/MIMIC-IV Project and eICU-CRD/longitudinal/mice_filled_baseline_preprocessed_out.csv', index=False)
...
pivot_df_scaled.to_csv('/Users/rishabhgoel/Desktop/Gen1E-RIDGE/MIMIC-IV Project and eICU-CRD/longitudinal/mice_filled_baseline_preprocessed_scaled_out.csv', index=False)
...
# Day 2
log_transformed_df = np.log(merged_df_day2.drop(columns=['subject_id', 'hadm_id']) + constant)

# Standardize the data
scaler = StandardScaler()
merged_df_day2_scaled= scaler.fit_transform(log_transformed_df)
merged_df_day2_scaled = pd.DataFrame(merged_df_day2_scaled, columns=(merged_df_day3.drop(columns=['subject_id', 'hadm_id'])).columns)

merged_df_day2_scaled.to_csv('/Users/rishabhgoel/Desktop/Gen1E-RIDGE/MIMIC-IV Project and eICU-CRD/longitudinal/mice_filled_baseline_preprocessed_scaled_out_day2.csv', index=False)


# Day 3
log_transformed_df = np.log(merged_df_day3.drop(columns=['subject_id', 'hadm_id']) + constant)

# Standardize the data
scaler = StandardScaler()
merged_df_day3_scaled= scaler.fit_transform(log_transformed_df)
merged_df_day3_scaled = pd.DataFrame(merged_df_day3_scaled, columns=(merged_df_day3.drop(columns=['subject_id', 'hadm_id'])).columns)

merged_df_day3_scaled.to_csv('/Users/rishabhgoel/Desktop/Gen1E-RIDGE/MIMIC-IV Project and eICU-CRD/longitudinal/mice_filled_baseline_preprocessed_scaled_out_day3.csv', index=False)
